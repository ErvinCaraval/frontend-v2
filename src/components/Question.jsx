...
export default function Question({ text, question, options, onSelect, selected }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <motion.div className="grid grid-cols-1 sm:grid-cols-2 gap-2" initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.04 } } }}>
        {options.map((opt, idx) => (
          <motion.button
            key={idx}
            ...
            className={`col-span-1 text-left w-full rounded-xl border px-4 py-3 text-sm md:text-base transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-bb-primary break-words ${selected === idx ? 'border-bb-primary bg-white/10' : 'border-white/20 bg-white/5 hover:bg-white/10'}`}
          >
